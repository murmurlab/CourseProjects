...
char	*strjoin_gnl_b(char *s1, char *s2)
{
	char	*return_str;
	size_t	i;
	size_t	j;
	...
	return_str = malloc (sizeof(char) * \
						(ft_strlen_gnl_b(s1) + ft_strlen_gnl_b(s2) + 1));
	i = -1;
	j = 0;
	while (s1[++i])
		return_str[i] = s1[i];
...
char	*get_new_buffer_b(char *buffer)
{
	...
	new_buffer = malloc(sizeof(char) * (ft_strlen_gnl_b(buffer) - i + 1));
	if (!new_buffer)
		return (NULL);
	i++;
	while (buffer[i])
